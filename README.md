...
1. **业务 / 全局人工层**：`🚀 手动切换`、`🤖 AI服务`、`🌍 国外流媒体`、`📺 哔哩哔哩`、`🍎 苹果服务`、`🌐 兜底策略`。这些组使用 `select`，并通过 `include-all-proxies=true` 直接暴露运行时注入的真实节点，支持各业务独立固定节点。
...
`🚀 手动切换` 继续作为全局手动入口，但不再承担所有业务的唯一人工选点状态。Bilibili 使用独立 `📺 哔哩哔哩` 策略，因此修改它不会再污染其他引用全局手动组的业务状态。
